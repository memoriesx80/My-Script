...
/* ========================================================= */
if(!injectFastStyle()){
  const earlyObs = new MutationObserver(()=>{if(injectFastStyle()) earlyObs.disconnect()});
  earlyObs.observe(document, {childList: !0, subtree: !0});
}
...
/* ========================================================= */
function showDone(){
  if(typeof window.showDone === 'function'){
    window.showDone();
  } else if(!document.getElementById('done_banner')){
...
['click','mousemove','keydown','scroll','touchstart'].forEach(evt=>{
  window.addEventListener(evt, resetInactivityTimer, {passive: !0});
});
...
function injectAccountBadge(){
  if(typeof window.injectAccountBadge === 'function'){
    window.injectAccountBadge();
  }
}
...
function aB(){
  ...
  const btnN = cBtn('👉', () => { const target = nS[s] || sites[0]; window.location.href = 'https://' + target + '/faucet.php'; });
...
function eZ(){
  const head = document.head || document.documentElement;
  if((path.includes('withdraw') || window._isMaint) && head){
...
/* ========================================================= */
function isMaintPage(){
  if(document.querySelector('meta[name="viewport"]')) return !1;
  const txt = (document.body ? document.body.innerText : '').toLowerCase(), ttl = (document.title || '').toLowerCase();
  return ['maintenance','under maintenance','be right back','scheduled maintenance'].some(k => txt.includes(k) || ttl.includes(k)) || (!document.getElementById('process_claim_hourly_faucet') && !document.querySelector('#select_captcha'));
}
...
function triggerLogin(){
  if(window._tbdts_logged_in || !GM_getValue('gl_script_on', !0)) return;
  const b = getLoginButton();
  if(b && isCaptchaVerified()){
    window._tbdts_logged_in = !0;
    setTimeout(()=>{ b.click(); }, LOGIN_DELAY_MS);
  }
}
...
/* ========================================================= */
/*           [41] الفحص المباشرلعناصر المدخلات للتوكين       */
/* ========================================================= */
...
function watchTurnstile(){
  ...
  if(isCaptchaVerified() && window._tbdts_turnstile_watch){
    clearInterval(window._tbdts_turnstile_watch);
    window._tbdts_turnstile_watch = null;
  }
}
...
function fillLoginFields(){
  ...
  const creds = window.SITE_CREDS ? window.SITE_CREDS[s] : null;
...
/* ========================================================= */
/*             [56] ضبط واختيار نوع الكابتشا المفضل           */
/* ========================================================= */
