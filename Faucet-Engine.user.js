// ==UserScript==
// @name        TBDTS V97 - Universal Engine
// @version      1997.11.13
// @match       *://tronpick.io/*
// @match       *://bnbpick.io/*
// @match       *://dogepick.io/*
...
/* ========================================================= */
/*              [1] قراءة بيانات تسجيل الدخول الديناميكية     */
/* ========================================================= */
...
'use strict';
/* ========================================================= */
/*              [3] منع إعادة التشغيل المزدوج                 */
/* ========================================================= */
if(window._m_run_active)return;
window._m_run_active=!0;

/* ========================================================= */
/*             [4] تعيين المتغيرات والجداول الديناميكية       */
/* ========================================================= */
const path=window.location.pathname,s=window.location.hostname;

// استخراج قائمة المواقع المتاحة بقلب هاد المتصفح حصراً
const sites = Object.keys(SITE_CREDS);

// دالة لمعرفة الموقع التالي بناءً على القائمة الديناميكية
function getNextSiteHost(currentHost) {
  const idx = sites.indexOf(currentHost);
  if (idx !== -1 && idx < sites.length - 1) {
    return sites[idx + 1];
  }
  return null;
}

// دالة لمعرفة الموقع السابق
function getPrevSiteHost(currentHost) {
  const idx = sites.indexOf(currentHost);
  if (idx > 0) {
    return sites[idx - 1];
  }
...
/* ========================================================= */
if(!injectFastStyle()){const earlyObs=new MutationObserver(()=>{if(injectFastStyle())earlyObs.disconnect()});earlyObs.observe(document,{childList:!0,subtree:!0})}

/* ========================================================= */
/*             [8] فحص حالة قفل الموقع الحالي                 */
/* ========================================================= */
function isUnlocked(dom){const lT=GM_getValue('f_lock_'+dom,0);return!lT||(Date.now()-lT>=36e5)}

/* ========================================================= */
/*             [9] دالة إعادة تحميل الصفحة القسرية            */
/* ========================================================= */
function hR(){window.location.replace(window.location.origin+window.location.pathname+'?t='+Date.now())}
...
/* ========================================================= */
function handleSuccess(){GM_setValue('err_cnt_'+s,0)}
...
['click','mousemove','keydown','scroll','touchstart'].forEach(evt=>{window.addEventListener(evt,resetInactivityTimer,{passive:!0})});

/* ========================================================= */
/*       [15] رصد عبارات الحماية والتمرير التلقائي للشاشة     */
/* ========================================================= */
let _phraseScrolled=!1;
...
function createNavContainers(){
  ...
  rC.style.cssText='position:fixed!important;bottom:8px!important;right:10px!important;display:flex!important;gap:12px!important;z-index:999999999!important;align-items:center!important;';
  const lC=document.createElement('div');
  lC.style.cssText='position:fixed!important;bottom:8px!important;left:10px!important;display:flex!important;gap:12px!important;z-index:999999999!important;align-items:center!important;';
  return {rC, lC};
}
...
function aB(){
  ...
  let audioMuted=GM_getValue('gl_audio_muted',!0);
  ...
  // التنقل بين المواقع المتاحة بقلب هاد المتصفح حصراً
  const btnP=cBtn('👈',()=>{
    const prev = getPrevSiteHost(s);
    if(prev) window.location.href='https://'+prev+'/faucet.php';
  });
  const btnN=cBtn('👉',()=>{
    const nxt = getNextSiteHost(s) || sites[0];
    if(nxt) window.location.href='https://'+nxt+'/faucet.php';
  });
...
/* ========================================================= */
const playAudio=t=>{
  if(GM_getValue('gl_audio_muted',!0))return;
...
/* ========================================================= */
window._tbdts_captcha_verified=!1;window._tbdts_verified_token=null;window._tbdts_turnstile_hooked=!1;window._tbdts_turnstile_watch=null;window._tbdts_claimed=!1;window._tbdts_logged_in=!1;window._tbdts_claim_scheduled=!1;window._tbdts_request_sent=!1;
...
function triggerClaim(){
  if(window._tbdts_claimed||!isUnlocked(s)||!GM_getValue('gl_script_on',!0))return;
  const b=getClaimButton();
  if(b && isCaptchaVerified()){
    window._tbdts_claimed=!0;
    window._tbdts_request_sent=!0;
    b.click();
  }
}
...
/* ========================================================= */
function triggerLogin(){if(window._tbdts_logged_in||!GM_getValue('gl_script_on',!0))return;const b=getLoginButton();if(b&&isCaptchaVerified()){window._tbdts_logged_in=!0;setTimeout(()=>{b.click()},LOGIN_DELAY_MS)}}
...
function dispatchCaptchaAction(){
  if(path.includes('login')||getLoginButton()){
    ...
    if(!window._tbdts_claim_scheduled){
      window._tbdts_claim_scheduled=!0;
      unlockClaimButton();
...
function hookTurnstileCallback(){
  ...
  if(uWin.turnstile&&!window._tbdts_turnstile_hooked){
    ...
    if(typeof origRender==='function'){
      window._tbdts_turnstile_hooked=!0;
...
if(document.documentElement){claimGuardObserver.observe(document.documentElement,{childList:!0,subtree:!0})}
...
function fillEmailInput(emailInP, emailVal){
  if(emailVal&&emailInP.value!==emailVal){
    ...
    emailInP.dispatchEvent(new Event('input',{bubbles:!0}));
    emailInP.dispatchEvent(new Event('change',{bubbles:!0}));
  }
}
...
function fillPasswordInput(passInP, passVal){
  if(passVal&&passInP.value!==passVal){
    ...
    passInP.dispatchEvent(new Event('input',{bubbles:!0}));
    passInP.dispatchEvent(new Event('change',{bubbles:!0}));
  }
}
...
function fillLoginFields(){
  const emailInP=document.getElementById('user_email'),passInP=document.getElementById('password'),creds=SITE_CREDS[s];
  if(emailInP&&passInP&&creds&&!window._loginHandled){
    window._loginHandled=!0;
...
function simulateFirstClick(){
  try{
    document.dispatchEvent(new MouseEvent('click',{bubbles:!0,cancelable:!0,view:window,clientX:Math.floor(Math.random()*window.innerWidth),clientY:Math.floor(Math.random()*window.innerHeight)}));
  }catch(e){}
}
...
function selectPreferredCaptcha(){
  ...
  if(sl&&sl.value!=='3'){
    sl.value='3';
    sl.dispatchEvent(new Event('change',{bubbles:!0}));
    sl.dispatchEvent(new Event('input',{bubbles:!0}));
  }
}
...
function autoFillWithdrawAmount(){
  if(!window._mCSet){
    ...
    if(am&&bal&&bal.innerText.trim()){
      ...
      if(numMatch){
        am.value=numMatch[0];
        am.dispatchEvent(new Event('input',{bubbles:!0}));
        am.dispatchEvent(new Event('change',{bubbles:!0}));
        window._mCSet=!0;
      }
    }
  }
...
function autoFillWithdrawAddress(){
  if(!window._aCplSet){
    ...
    if(ad&&cp){
      ...
      if(val){
        ad.value=val;
        ad.dispatchEvent(new Event('input',{bubbles:!0}));
        ad.dispatchEvent(new Event('change',{bubbles:!0}));
        window._aCplSet=!0;
      }
    }
  }
...
function handleWithdrawLogic(){
  if(path.includes('withdraw')){
    ...
    if(cfInput&&cfInput.value&&cfInput.value.length>30){
      ...
      window._tbdts_captcha_verified=!0;
    }
  }
}
...
function navigateNextOnSuccess(){
  const nxt = getNextSiteHost(s);
  if(nxt){
    setTimeout(()=>{ window.location.href='https://'+nxt+'/faucet.php'; }, 1000);
  } else {
    showDone();
  }
}
